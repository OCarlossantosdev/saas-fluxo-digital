    ...
    async function initDashboard() {
      try {
        ...
        const [clientsRes, projectsRes, tasksRes, transRes] = await Promise.all([
          ...
          supabase.from("client_transactions").select("amount, type, date")
        ]);
        ...
        financeiro.forEach((f: any) => {
          if (f.type === 'receita') fatTotal += Number(f.amount);
        });
        ...
        setChartData(meses.map(m => ({ name: m, receita: Math.random() * 5000, lucro: Math.random() * 3000 })));
